...
class ApiService {
  ...
  async loginWithLinkedIn(linkedinData: {
    ...
    console.log('API Service - loginWithLinkedIn called with:', linkedinData);
    const response = await this.request('/auth/linkedin', {
      method: 'POST',
    ...
    if (response.success && response.data && typeof response.data === 'object' && 'token' in response.data) {
      const tokenData = response.data as any;
      console.log('LinkedIn login successful, setting session with token');
      sessionService.setSession(
        tokenData.token,
        tokenData.refreshToken,
        tokenData.expiresIn || 3600
      );
      toast.success('You have been successfully logged in with LinkedIn');
    } else {
