...
export const Blogs: React.FC = () => {
  ...
  const categories = getBlogCategories().map(cat => ({
    name: cat.name,
    icon: cat.name === 'All' ? BookOpen :
          cat.name === 'Career Tips' ? TrendingUp :
          cat.name === 'Skill Building' ? GraduationCap :
          cat.name === 'Workplace Trends' ? Briefcase :
          cat.name === 'Productivity' ? Target :
          cat.name === 'Employer Insights' ? Users : BookOpen,
    count: cat.count
  }));

  // Get blog posts from unified data source
  const blogPosts = getBlogsForBlogsPage({
                ...
                <div className="space-y-4">
                  <div className="flex items-center space-x-4 text-sm text-neutral-600 dark:text-neutral-400">
                    ...
                    <div className="flex items-center space-x-1">
                      ...
                      <span>{formatBlogDate(featuredPost.publishDate)}</span>
                    </div>
                    <div className="flex items-center space-x-1">
                  ...
                  <div className="text-sm text-neutral-500 dark:text-neutral-400">
                    {formatBlogViews(featuredPost.views)}
                  </div>
                </div>
              </div>
            </div>
      ...
      <div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {recentPosts.map((post) => (
            <ElevatedCard key={post.id} className="overflow-hidden">
