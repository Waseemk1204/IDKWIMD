...
import BlogBookmark from '../models/BlogBookmark';
...
// Toggle like blog
export const toggleLikeBlog = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    const existingLike = await BlogLike.findOne({ user: userId, blog: id });
    ...
    if (existingLike) {
      ...
      res.json({
        ...
        data: { liked: false, likes: blog.likes - 1 }
      });
    } else {
      // Like: Add the like and increment count
      const newLike = new BlogLike({ user: userId, blog: id });
      await newLike.save();
      await Blog.findByIdAndUpdate(id, { $inc: { likes: 1 } });
      ...
      res.json({
        ...
        data: { liked: true, likes: blog.likes + 1 }
      });
    }
  } catch (error) {
...
export const toggleBookmarkBlog = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const userId = req.user._id;
    ...
    const existingBookmark = await BlogBookmark.findOne({ user: userId, blog: id });
    ...
    if (existingBookmark) {
      ...
      // Add bookmark
      const newBookmark = new BlogBookmark({ user: userId, blog: id });
...
// Get user's blog interactions (likes and bookmarks)
export const getUserBlogInteractions = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const userId = req.user._id;
    ...
    if (!Array.isArray(blogIds)) {
      res.status(400).json({
        success: false,
        message: 'blogIds must be an array'
      });
      return;
    }

    const [likes, bookmarks] = await Promise.all([
      BlogLike.find({ user: userId, blog: { $in: blogIds } }).select('blog'),
      BlogBookmark.find({ user: userId, blog: { $in: blogIds } }).select('blog')
    ]);

    const likedBlogs = likes.map(like => like.blog.toString());
    const bookmarkedBlogs = bookmarks.map(bookmark => bookmark.blog.toString());
    ...
    console.error('Get user blog interactions error:', error);
    res.status(500).json({
      success: false,
      message: 'Internal server error'
    });
